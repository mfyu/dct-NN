...
def forward(TRAIN_STEPS):
    label = np.loadtxt('frame39_quantized_labels.gz')
    data = np.loadtxt('frame39_data.gz')

    test_label = np.loadtxt('frame0_quantized_labels.gz')
    test_data = np.loadtxt('frame0_data.gz')
    ...
    x_train, y_train, x_test, y_test = TRAIN_TEST_SIZE(388000)
...
def reverse(TRAIN_STEPS, train):# weights, biases):
  ...
  #      'out': tf.Variable(tf.random_normal([n_outputs])),
   ...
   # TRAIN_STEPS = 100
    ...
    psnrfile = open('forwardreverse/psnr.txt', 'a')
    psnrfile.write(f'Train Steps: {TRAIN_STEPS}, PSNR: {psnr(im,recon)}\n')
    psnrfile.close()

    Image.fromarray(np.uint8(recon)).save('test/old/frame0_recon_quantize_'+str(TRAIN_STEPS)+'step_foward_reverse.bmp')
...
for trainstep in range(200,4100,200):
    reverse(trainstep,prediction)
